...
Successfully restored ${res.importedCount} notes out of ${res.total} notes in file.`);
  ...
  function updateSignerModalUI() {
    ...
    if (isNip07) {
      ...
      if (isNip07Available()) {
        nip07InfoText.textContent = "\u{1F98A} NIP-07 Extension Available (nos2x-fox / Alby)";
...
Click for Settings & Device Pairing`;
  ...
  function createNoteItemCard(note) {
    ...
    <div class="note-item-header">
      <span class="note-title">${note.pinned ? "\u{1F4CC} " : ""}${escapeHtml(displayTitle)}</span>
      ${reminderHtml}
    </div>
  ...
  function updateSyncBadge(status) {
    ...
    if (status.syncState === "synced") {
      ...
      syncStatusText.textContent = `\u{1F512} Encrypted (${signerName}) & Synced (${status.connectedCount}/${status.totalRelays} relays)`;
    } else if (status.syncState === "syncing") {
      dot.className = "sync-dot syncing";
      syncStatusText.textContent = `\u{1F504} Syncing (${signerName})...`;
    } else {
