(() => {
  ...
  async function executeNip07Method(methodName, args = []) {
    const extensionApi2 = typeof browser !== "undefined" ? browser : typeof chrome !== "undefined" ? chrome : null;
    if (typeof window !== "undefined" && window.nostr) {
      const fn = getNestedMethod(window.nostr, methodName);
      if (fn) return await fn(...args);
    }
    if (extensionApi2 && extensionApi2.runtime && extensionApi2.runtime.sendMessage) {
      const res = await new Promise((resolve) => {
        extensionApi2.runtime.sendMessage({
          type: "NIP07_REQUEST",
          method: methodName,
          args
  ...
  function isNip07Available() {
    ...
    const extensionApi2 = typeof browser !== "undefined" ? browser : typeof chrome !== "undefined" ? chrome : null;
    return !!(extensionApi2 && extensionApi2.runtime);
  }
  ...
  async function exportVaultJSON(notes, settings, { isAuto = false, force = false } = {}) {
    ...
    const extensionApi2 = typeof browser !== "undefined" && browser.downloads ? browser : typeof chrome !== "undefined" && chrome.downloads ? chrome : null;
    if (extensionApi2 && extensionApi2.downloads && typeof extensionApi2.downloads.download === "function") {
      try {
        ...
        const result = extensionApi2.downloads.download(downloadOptions);
...
> *Your privacy is fully preserved. No server reads your unencrypted notes!*`,
  ...
  extensionApi.runtime.onInstalled.addListener(() => {
    extensionApi.contextMenus.create({
      ...
      title: "Add selection to Monstr Note \u{1F47E}\u{1F512}",
    ...
    if (request && request.type === "NIP07_REQUEST") {
      (async () => {
        try {
          ...
          const activeTab = tabs.find((t) => t.active && t.url && (t.url.startsWith("http://") || t.url.startsWith("https://"))) || tabs.find((t) => t.url && (t.url.startsWith("http://") || t.url.startsWith("https://"))) || tabs.find((t) => t.active) || tabs[0];
