(() => {
  ...
  // src/lib/crypto/nostr-crypto.js
  async function executeNip07Method(methodName, args = []) {
  ...
  function createNewKeypair() {
    ...
    const secretHex = bytesToHex3(secretKey);
    const nsec = nip19_exports.nsecEncode(secretKey);
  ...
  function parseSecretKey(input) {
    ...
    if (cleanInput.startsWith("nsec1")) {
      try {
        const decoded = nip19_exports.decode(cleanInput);
        if (decoded.type === "nsec") {
          const sk = decoded.data;
          const secretHex = bytesToHex3(sk);
    ...
    if (/^[0-9a-fA-F]{64}$/.test(cleanInput)) {
      const sk = hexToBytes2(cleanInput);
      const pubkey = getPublicKey(sk);
      return {
  ...
  function getSelfConversationKey(secretKeyHex) {
    const sk = hexToBytes2(secretKeyHex);
    const pk = getPublicKey(sk);
    return nip44_exports.v2.utils.getConversationKey(sk, pk);
  }
  function normalizeSignerConfig(signerOrSecretKey) {
    if (typeof signerOrSecretKey === "string") {
      const sk = hexToBytes2(signerOrSecretKey);
      return {
        type: "vault",
    ...
    if (signerOrSecretKey && !signerOrSecretKey.pubkeyHex && signerOrSecretKey.secretKeyHex) {
      const sk = hexToBytes2(signerOrSecretKey.secretKeyHex);
      return {
        type: "vault",
  ...
  async function encryptPayload(plaintext, signerOrSecretKey) {
  ...
  async function decryptPayload(ciphertext, signerOrSecretKey) {
    const config = normalizeSignerConfig(signerOrSecretKey);
    if (config.type === "nip07") {
      return await executeNip07Method("nip44.decrypt", [config.pubkeyHex, ciphertext]);
    }
    const convKey = getSelfConversationKey(config.secretKeyHex);
    return nip44_exports.v2.decrypt(ciphertext, convKey);
  }
  async function createSignedEvent({ kind, content, tags = [], signerConfig, secretKeyHex, createdAt = Math.floor(Date.now() / 1e3) }) {
    ...
    const sk = hexToBytes2(config.secretKeyHex);
  ...
  function renderMarkdown(markdownText = "") {
    ...
    html = lines.map((line) => {
      line = line.trim();
      if (line.startsWith("<h1") || line.startsWith("<h2") || line.startsWith("<h3") || line.startsWith("<pre") || line.startsWith("<blockquote") || line.startsWith("<ul") || line.startsWith("<ol") || line.startsWith('<div class="task-list-item')) {
        return line;
      }
  ...
  function parseInlineStyle(text) {
    return text.replace(/\*\*([^*]+)\*\*/g, "<strong>$1</strong>").replace(/\*([^*]+)\*/g, "<em>$1</em>").replace(/~~([^~]+)~~/g, "<del>$1</del>").replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" target="_blank" rel="noopener noreferrer">$1</a>');
  }
  function toggleChecklistItem(markdownText, itemIndex) {
    let currentIndex = 0;
  ...
  async function exportVaultJSON(notes, settings, { isAuto = false, force = false } = {}) {
    ...
    const filename = isAuto ? "monstr-note-backup-auto.json" : `monstr-note-vault-backup-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.json`;
    const vaultData = {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        notes = data.manifest.map((item) => ({
          ...
          content: (contents[item.id] !== void 0 ? contents[item.id] : item.content) || "",
      ...
      } else if (data && typeof data === "object" && (data.id || data.content !== void 0 || data.title !== void 0)) {
        notes = [data];
      } else {
        throw new Error("Invalid backup file format: missing notes array.");
      }
      const validNotes = notes.filter((n) => n && typeof n === "object" && (n.id || n.content !== void 0 || n.title !== void 0));
      return {
        success: true,
        notes: validNotes,
  ...
  // src/lib/storage/notes-store.js
  var STORAGE_KEYS = {
    NOTES: "ff_notes_data",
  ...
  function deriveTitleFromContent(content) {
    ...
    const clean3 = firstLine.replace(/^#+\s*/, "").replace(/^[-*+]\s+\[[ xX]\]\s*/, "").replace(/^[-*+]\s*/, "").replace(/[`*_~>]/g, "").trim();
    if (!clean3) return "";
    return clean3.length > 50 ? clean3.substring(0, 50) + "..." : clean3;
  }
  ...
  var NotesStore = class {
    constructor() {
      ...
      this.pendingDeletions = /* @__PURE__ */ new Map();
      this.keypair = null;
    ...
    createWelcomeNote() {
      ...
      return {
        ...
        content: `# Welcome to Monstr Note \u{1F47E}\u{1F512}
...
> *Your privacy is fully preserved. No server reads your unencrypted notes!*`,
    ...
    async saveNotes(immediate = false) {
      ...
      if (immediate || typeof process !== "undefined" && process.env && false) {
    ...
    async importBackupNotes(importedNotes) {
      ...
      for (const rawNote of importedNotes) {
        ...
        const noteObj = {
          ...
          created_at: typeof rawNote.created_at === "number" ? rawNote.created_at : now2,
          updated_at: now2 + timeOffset++,
    ...
    async updateNote(id, changes) {
      ...
      if (changes.title !== void 0) {
        ...
        if (trimmed.length > 0) {
          ...
          isManual = false;
          newTitle = deriveTitleFromContent(changes.content !== void 0 ? changes.content : existing.content);
        }
      } else if (changes.content !== void 0 && !isManual) {
        newTitle = deriveTitleFromContent(changes.content);
      }
      const now2 = Date.now();
      const newUpdatedAt = Math.max(now2, (existing.updated_at || 0) + 1);
      const updated = {
        ...existing,
        ...changes,
        title: newTitle,
    ...
    async checkReminders() {
      ...
      for (let i2 = 0; i2 < this.notes.length; i2++) {
        const note = this.notes[i2];
        if (note.reminder_at && !note.reminder_triggered && now2 >= note.reminder_at) {
          note.reminder_triggered = true;
          note.updated_at = Math.max(now2, maxTimestamp + timeOffset++);
          this.notes.splice(i2, 1);
          this.notes.unshift(note);
    ...
    async mergeRemoteNote(remoteNote) {
      ...
      if (remoteNote.deleted) {
        ...
        await this.saveDeletedIds();
        const existingIndex2 = this.notes.findIndex((n) => n.id === remoteNote.id);
        if (existingIndex2 !== -1) {
          this.notes.splice(existingIndex2, 1);
          await this.saveNotes();
  ...
  var SyncEngine = class {
    constructor() {
      ...
      this.listeners = /* @__PURE__ */ new Set();
      this.publishedEventsMap = /* @__PURE__ */ new Map();
      this.currentPubkey = null;
      this.currentRelaysStr = "";
      this.storeChangeTimer = null;
      this.publishingNotes = /* @__PURE__ */ new Set();
    }
    async init() {
      this.pool.onStatusChange(({ connectedCount, totalRelays }) => {
        this.notify();
      });
      notesStore.subscribe((state2) => {
        this.signerConfig = state2.signerConfig;
        const newRelaysStr = Array.isArray(state2.relays) ? state2.relays.join(",") : "";
        if (newRelaysStr !== this.currentRelaysStr) {
          this.currentRelaysStr = newRelaysStr;
          this.pool.updateRelays(state2.relays || []);
        }
        const newPubkey = state2.signerConfig?.pubkeyHex || null;
        if (newPubkey && newPubkey !== this.currentPubkey) {
          this.currentPubkey = newPubkey;
          this.startSubscription();
        }
        this.scheduleStoreChangeCheck(state2);
      });
    ...
    async onStoreChange(state2) {
      ...
      if (state2.pendingDeletions && state2.pendingDeletions.size > 0) {
        for (const [id, tombstone] of state2.pendingDeletions.entries()) {
    ...
    async publishNote(note) {
      ...
      try {
        ...
        const signedEvent = await createSignedEvent({
          ...
          createdAt: Math.floor(note.updated_at / 1e3)
        });
  ...
  var state = null;
  var currentViewMode = "edit";
  var currentPanel = "list";
  var searchQuery = "";
  var viewNotesList = document.getElementById("view-notes-list");
  var viewNoteEditor = document.getElementById("view-note-editor");
  var headerListTitle = document.getElementById("header-list-title");
  var btnBackToList = document.getElementById("btn-back-to-list");
  var mainNotesListEl = document.getElementById("main-notes-list");
  ...
  var markdownPreviewEl = document.getElementById("note-markdown-preview");
  var wordCountEl = document.getElementById("word-count");
  var syncStatusBadge = document.getElementById("sync-status-badge");
  var syncStatusText = document.getElementById("sync-status-text");
  var syncStatusBadgeList = document.getElementById("sync-status-badge-list");
  var syncStatusTextList = document.getElementById("sync-status-text-list");
  var btnMoreMenu = document.getElementById("btn-more-menu");
  var moreMenuDropdown = document.getElementById("more-menu-dropdown");
  var syncModal = document.getElementById("sync-modal");
  var btnSyncSettings = document.getElementById("btn-sync-settings");
  var btnCloseModal = document.getElementById("btn-close-modal");
  var keyDisplay = document.getElementById("key-display");
  var btnCopyKey = document.getElementById("btn-copy-key");
  var inputImportKey = document.getElementById("input-import-key");
  var btnImportKey = document.getElementById("btn-import-key");
  var radioVault = document.getElementById("signer-mode-vault");
  var radioNip07 = document.getElementById("signer-mode-nip07");
  var sectionVaultDetails = document.getElementById("section-vault-details");
  var sectionNip07Status = document.getElementById("section-nip07-status");
  var nip07InfoText = document.getElementById("nip07-info-text");
  var nip07PubkeyDisplay = document.getElementById("nip07-pubkey-display");
  var btnConnectNip07 = document.getElementById("btn-connect-nip07");
...
Pubkey: ${res.npub}`);
    ...
    btnCopyKey?.addEventListener("click", () => {
      if (state?.keypair) {
        ...
        setTimeout(() => {
          if (btnCopyKey) btnCopyKey.textContent = "\u{1F4CB} Copy Sync Key";
        }, 2e3);
      }
    });
...
Successfully restored ${res.importedCount} notes out of ${res.total} notes in file.`);
    ...
    document.querySelectorAll(".btn-timer-preset").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        const reminderAt = Date.now() + mins * 60 * 1e3;
  ...
  var groupByMode = "none";
  var activeTagFilter = null;
  var COLOR_NAMES = {
    "fox-accent": "\u{1F49C} Purple",
    "default": "\u{1F49C} Purple",
    "red": "\u{1F534} Crimson Red",
    "yellow": "\u{1F7E1} Amber Gold",
    "green": "\u{1F7E2} Mint Green",
    "blue": "\u{1F535} Cyan Blue",
    "purple": "\u{1F7E3} Deep Violet"
  };
  function renderNotesList() {
    ...
    if (searchQuery) {
      filtered = filtered.filter(
        (n) => n.title.toLowerCase().includes(searchQuery) || n.content.toLowerCase().includes(searchQuery) || Array.isArray(n.tags) && n.tags.some((t) => t.toLowerCase().includes(searchQuery))
      );
    }
    if (activeTagFilter) {
  ...
  function createNoteItemCard(note) {
    ...
    const tagsHtml = Array.isArray(note.tags) && note.tags.length > 0 ? `<div class="tags-list-inline">${note.tags.map((t) => `<span class="tag-chip" data-tag="${escapeHtml(t)}">#${escapeHtml(t)}</span>`).join("")}</div>` : "";
    let reminderHtml = "";
    if (note.reminder_at) {
    ...
    setSafeHTML(itemEl, `
    <div class="drawer-item-title">${note.pinned ? "\u{1F4CC} " : ""}${escapeHtml(displayTitle)} ${reminderHtml}</div>
  ...
  function renderGroupedByTag(notesList) {
    ...
    for (const [tag, notes] of groups.entries()) {
      ...
      setSafeHTML(header, `<span>\u{1F3F7}\uFE0F #${escapeHtml(tag)}</span> <span class="group-count">(${notes.length})</span>`);
    ...
    if (untagged.length > 0) {
      const header = document.createElement("div");
      header.className = "note-group-header";
      setSafeHTML(header, `<span>\u{1F3F7}\uFE0F Untagged</span> <span class="group-count">(${untagged.length})</span>`);
      mainNotesListEl.appendChild(header);
  ...
  function renderGroupedByColor(notesList) {
    ...
    for (const [colorKey, notes] of groups.entries()) {
      const header = document.createElement("div");
      header.className = "note-group-header";
      const colorLabel = COLOR_NAMES[colorKey] || `\u{1F3A8} ${colorKey}`;
  ...
  function getDateGroupName(ts) {
    if (!ts) return "Older \u{1F4C5}";
    const date = new Date(typeof ts === "number" && ts < 1e10 ? ts * 1e3 : ts);
    if (isNaN(date.getTime())) return "Older \u{1F4C5}";
    ...
    if (noteTime >= todayStart) return "Today \u{1F4C5}";
    if (noteTime >= yesterdayStart) return "Yesterday \u{1F4C5}";
    if (noteTime >= weekStart) return "This Week \u{1F4C5}";
    return "Older \u{1F4C5}";
  }
  function renderActiveNote() {
    ...
    if (btnPin) btnPin.textContent = note.pinned ? "\u{1F4CC} Unpin Note" : "\u{1F4CC} Pin Note";
    ...
    if (noteTagsChips) {
      setSafeHTML(noteTagsChips, Array.isArray(note.tags) && note.tags.length > 0 ? note.tags.map((t) => `<span class="tag-chip" data-tag="${escapeHtml(t)}">#${escapeHtml(t)}</span>`).join("") : "");
    }
    const btnReminderPicker = document.getElementById("btn-reminder-picker");
  ...
  function updateSignerModalUI() {
    ...
    if (isNip07) {
      ...
      if (isNip07Available()) {
        if (nip07InfoText) nip07InfoText.textContent = "\u{1F98A} NIP-07 Extension Available (nos2x-fox / Alby)";
  ...
  function generateIdenticonSvg(pubkeyHex, isNip07, pictureUrl) {
    ...
    const h2 = (h1 + 135) % 360;
    const badge = isNip07 ? '<span class="user-avatar-badge" title="nos2x-fox">\u{1F98A}</span>' : '<span class="user-avatar-badge" title="Vault">\u{1F512}</span>';
    const svgFallback = `
...
Click for Settings & Device Pairing`;
  ...
  function updateSyncBadge(status) {
    ...
    const text = status.syncState === "synced" ? `\u{1F512} Encrypted (${signerName})` : status.syncState === "syncing" ? `\u{1F504} Syncing...` : "\u26A0\uFE0F Offline";
    if (syncStatusBadge) {
  ...
  function extractSnippet(content) {
    if (!content) return "";
    const clean3 = content.replace(/^#+\s+/gm, "").replace(/^[-*+]\s+\[[ xX]\]\s+/gm, "").replace(/^[-*+]\s+/gm, "").replace(/[`*_~>]/g, "").replace(/\s+/g, " ").trim();
    return clean3.length > 70 ? clean3.substring(0, 70) + "..." : clean3;
  }
